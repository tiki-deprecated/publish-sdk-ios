            ...
            try TikiSdk.config().initialize(publishingId: publishingId, id:id, onComplete: {
                Task{
                    do{
                        ...
                        XCTAssertEqual(license.title.tags[0].value, TagCommon.advertisingData.rawValue)
                    }catch{
                        XCTFail()
                    }
                }
        ...
        do{
            try TikiSdk.config().initialize(publishingId: publishingId, id:id, onComplete: {
                Task{
                    ...
                    let guardResult = try await TikiSdk.guard(ptr: "source", usecases:[Usecase(UsecaseCommon.support)], destinations: [])
